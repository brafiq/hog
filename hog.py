...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def announce(score0,score1):
        prevscore = previous_score
        prevhigh = previous_high
        if who == 0:
            score = score0
        if who == 1:
            score = score1
        gain = score - prevscore
        if gain>prevhigh:
            if gain == 1:
            ...
            prevhigh = gain
        return announce_highest(who,prevhigh,score)
    return announce
    # END PROBLEM 7
...
def make_averaged(fn, num_samples=1000):
    ...
    def averageval(*args):
        tempsum = 0
        numsamps = num_samples
        while numsamps > 0:
            tempsum += fn(*args)
            numsamps -= 1
        average = tempsum / num_samples
        return average
    return averageval
    # END PROBLEM 8
...
def bacon_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    """
    # BEGIN PROBLEM 10
    addedscore = free_bacon(opponent_score)
    if addedscore >= margin:
        num_rolls = 0
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    if is_swap(newscore,opponent_score) == True:
        num_rolls = 0
    if addedscore >= margin:
        num_rolls = 0
    return num_rolls  # Replace this statement
